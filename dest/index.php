...
if (isset($_GET['id']) && !empty($_GET['id'])) {
  $stmt = $pdo->prepare("SELECT count(*) as nb FROM destination WHERE destination_id = :id");
...
<body data-id=<?= $id ?>>
  ...
  <?php
  $stmt = $pdo->prepare("SELECT * FROM destination WHERE destination_id = :id");
  $stmt->execute(['id' => $id]);
  ...
  <section class="destination-about">
    <div class="about-content">
      <h1>
        ...
        <?= $destination['destination_surname'] ?>
      </h1>
      <h3>
        <?= $destination['little_desc'] ?>
      </h3>
      <p>
        <?= $destination['big_desc'] ?>
      </p>
    </div>
    <div class="image">
  ...
  <section class="destination-activities">
    <div class="activity-container">
      ...
      <ul>
        <?php
        $stmt = $pdo->prepare("SELECT * FROM activity WHERE destination_id = :id");
        $stmt->execute(['id' => $id]);
  ...
  <section class="offers" id="offers">
    ...
    <div class="offers-container">
      ...
      <?php $stmt = $pdo->prepare("SELECT * FROM travel_offer o, destination d WHERE o.destination_id = d.destination_id AND o.destination_id = :id ORDER BY travel_offer_id");
      $stmt->execute(['id' => $id]);
